...
class JacoMujocoEnvUtil:
    ...
    def _create_init_angle(self):
        if self.task in ['reaching', 'picking', 'pickAndplace']:
            random_init_angle = [uniform(0, 0.5), uniform(3.8,4), uniform(
    ...
    def _get_reward(self):
        if self.reward_method is None:
            if self.task == 'reaching':
                ...
                wb_th = 0.15
                wb = np.linalg.norm(
                        self.__get_property('EEE', 'position')[0] - self.base_position[0])
                if wb < wb_th:
                    reward -= (wb_th - wb)
                # Gripper too low
                z_th = 0.1
...
class PandaMujocoEnvUtil:
    ...
    def _get_observation(self):
        ...
        #print("pose : {}  , orientation :  {} ".format(self.gripper_pose[0][:3],self.gripper_pose[0][3:6]))
        return np.array(observation, dtype=np.float32).flatten()
    ...
    def _take_action(self, a, weight=None, subgoal=None, id=None):
        ...
        if self.rulebased_subgoal:
            ...
            subgoal_reach = subgoal['level1_reaching/level0'][0] + self.target_pos    #분홍색 
        ...
        #self.target_pos = self.gripper_pose[0] + np.hstack([a[:3]/25, a[3:6]/5])
        self.target_pos = self.gripper_pose[0] + np.hstack([a[:3]/25, a[3:6]/5])             ##########################
        print(subgoal_reach[3:6])
        # self.target_pos[3]= -3.04
        # self.target_pos[4]= -0.28
        # self.target_pos[5]= 0.8564
        self.target_pos[3]= -0.488025
        self.target_pos[4]= 0.067124
        self.target_pos[5]= 0.9830349
 #       self.target_pos=[ 0,  0.3842 , 0.3, -3.03970959 ,-0.28013236 , 0.85639528]
        #print(transformations.quaternion_from_euler(*self.target_pos[3:6], axes="rxyz"))
        if abs(self.target_pos[5]) > pi:
            self.target_pos[5] += -np.sign(self.target_pos[5])*2*pi
