...
class OSC(Controller):
    ...
    def __init__(
        ...
        if self.robot_config.N_ROBOTS == 1:
            self.end_effector = ["EEE"]
        else:
            self.end_effector = ["EEE_"+str(ii) for ii in range(1,self.robot_config.N_ROBOTS+1)]
...
class OSC_1(Controller):
    ...
    def __init__(
        ...
        super(OSC_1, self).__init__(robot_config)
    ...
    def generate(
        ...
        if target_velocity is None:
            target_velocity = self.ZEROS_SEVEN
        if ref_frame == "EE":
            ref_frame = self.end_effector
        ...
        for i in range(self.robot_config.N_ROBOTS):
            ...
            if np.all(target_velocity == 0):
                # if there's no target velocity in task space,
                # compensate for velocity in joint space (more accurate)
                u = -1 * self.kv * np.dot(M[i], dq[7*i:7*(i+1)])
                if debug:
                    print("u: ",u)
            else:
                dx = np.zeros(6)
                dx[self.ctrlr_dof] = np.dot(J[i][self.ctrlr_dof], dq[7*i:7*(i+1)])
                u_task += self.kv * (dx - target_velocity)
                if debug:
                    print("u_task5: ",u_task)
            ...
            # add in estimation of full centrifugal and Coriolis effects ----------
            if self.use_C:
                u -= np.dot(self.robot_config.C(q=q[7*i:7*(i+1)], dq=dq[7*i:7*(i+1)]), dq[7*i:7*(i+1)])
                if debug:
                    print("u3: ",u)
            ...
            # add in gravity term in joint space ----------------------------------
            if self.use_g:
                u -= self.robot_config.g(q=q)[7*i:7*(i+1)]
                if debug:
                    print("u4: ",u)
            ...
            if self.null_controllers is not None:
                for null_controller in self.null_controllers:
                    # generate control signal to apply in null space
                    u_null = null_controller.generate(q[7*i:7*(i+1)], dq[7*i:7*(i+1)])
                    # calculate null space filter
                    Jbar = np.dot(M_inv, np.dot(J[i][self.ctrlr_dof].T, Mx))
                    null_filter = self.IDENTITY_N_JOINTS - np.dot(J[i][self.ctrlr_dof].T, Jbar.T)
                    # add in filtered null space control signal
